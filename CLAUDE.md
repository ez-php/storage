...
tests/
  ...
  S3DriverTest.php            — live put/get/delete cycle against a real bucket; skipped without AWS credentials
  S3DriverOfflineTest.php     — presigned/custom URLs, path-traversal rejection, key encoding; dummy credentials, always runs
...
- **Only the live S3 cycle is skipped** by default (`S3DriverTest`, `markTestSkipped` when credentials are absent — set `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_BUCKET` to run it). Everything that needs no bucket — including the path-traversal guards — lives in `S3DriverOfflineTest` and always runs; keep new credential-free checks there, not behind the credential gate.
